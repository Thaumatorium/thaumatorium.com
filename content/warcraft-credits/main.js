...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadAndVisualizeD3(filename1, filename2) {
    console.log(
      `Main: Attempting D3 load via worker: ${filename1}, ${filename2}`
    );
    ...
    try {
      ...
      worker.onmessage = (event) => {
        ...
        if (status === "success" && graphData?.nodes) {
          ...
          if (Array.isArray(normalizedRolePositionsData)) {
            normalizedRolePositionsData.forEach(([role, positionObject]) => {
              if (
                role &&
                positionObject &&
                typeof positionObject.normX === 'number' &&
                typeof positionObject.normY === 'number'
              ) {
          ...
          if (!state.d3Simulation) {
             ...
             if (state.isShiftPressed) { // Apply shift state if held during load
      ...
      worker.onerror = (error) => {
        if (worker !== state.activeWorker) { return; } // Ignore errors from old workers
      ...
      if (state.d3Simulation) { state.d3Simulation.stop(); state.d3Simulation = null; }
  ...
  window.addEventListener("blur", () => { // Handle losing focus while shift is pressed
