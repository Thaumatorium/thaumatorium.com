 ...
 */
export function processDataForD3(
  jsonData1,
  jsonData2,
  filename1,
  filename2,
  ...
  if (!jsonData1)
    console.warn("jsonData1 is missing or invalid for:", filename1);
  if (!jsonData2)
    console.warn("jsonData2 is missing or invalid for:", filename2);
  ...
  if (jsonData1) {
    gameName1 =
      getGameNameFromData(jsonData1, filename1) || gameTitleMap[filename1];
    if (!gameName1) {
      gameName1 = filename1
        .replace(".json", "")
        .replace(/_/g, " ")
    ...
    if (gameName1) {
      ...
      if (Array.isArray(peopleArray1)) {
        peopleArray1.forEach((personObj) => {
          if (personObj?.name && typeof personObj.name === "string") {
            ...
            if (name) {
              ...
              const roles = Array.isArray(personObj.roles)
                ? personObj.roles.map((r) => String(r).trim()).filter(Boolean)
                : [];
              peopleMap1.set(name, {
        ...
        console.warn(
          `Game 1 data for '${gameName1}' is not an array in ${filename1}`
        );
      }
    } else {
      console.error(
        `Could not determine identity for Game 1 from ${filename1}.`
      );
    }
  } else {
  ...
  if (!isSameGame && jsonData2) {
    gameName2 =
      getGameNameFromData(jsonData2, filename2) || gameTitleMap[filename2];
    if (!gameName2) {
      gameName2 = filename2
        .replace(".json", "")
        .replace(/_/g, " ")
    ...
    if (gameName2) {
      ...
      if (gameId1 && gameId1 === gameId2) {
        console.warn(
          `Game 2 ('${gameName2}') resolved to the same ID as Game 1 ('${gameName1}'). Treating as single game view.`
        );
        isSameGame = true;
        gameName2 = gameName1;
        gameId2 = gameId1;
        ...
        if (Array.isArray(peopleArray2)) {
          ...
          console.warn(
            `Game 2 data for '${gameName2}' is not an array in ${filename2}`
          );
        }
      }
    } else {
      console.error(
        `Could not determine identity for Game 2 from ${filename2}.`
      );
  ...
  if (!gameId1 && !gameId2) {
    throw new Error(
      "Could not identify or load data for either selected game. Cannot process contributors."
    );
  }
  ...
  allPeopleNames.forEach((personName) => {
    ...
    if (isSameGame) {
      ...
      if (inGame1 && inGame2) {
        ...
        primaryRole =
          getPrimaryRole(personData1.roles) !== DEFAULT_ROLE
            ? getPrimaryRole(personData1.roles)
            : getPrimaryRole(personData2.roles);
      } else if (inGame1) {
    ...
    if (shouldProcessPerson) {
      primaryRole =
        primaryRole && primaryRole !== DEFAULT_ROLE
          ? primaryRole
          : getPrimaryRole(rolesForMap);
  ...
  links.forEach((link) => {
    ...
    if (sourceNode) {
      ...
      console.warn(
        `Degree calculation: Source node ${link.source} not found in nodesMap.`
      );
    }
    if (targetNode) {
      targetNode.degree++;
      ...
      console.warn(
        `Degree calculation: Target node ${link.target} not found in nodesMap.`
      );
    }
  });

  console.log(
    `D3 Processor: Created ${nodes.length} nodes and ${links.length} links.`
  );
  console.log(
    `D3 Processor: Role map contains ${personRolesMap.size} entries.`
  );
  return { nodes, links };
}
