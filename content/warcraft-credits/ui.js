...
export function createHandleSelectionChange(
  ...
  const { fileSelect1, fileSelect2, errorMessage, svgContainer, tooltip } =
    elements;
  ...
  return function handleSelectionChange() {
    ...
    if (fileSelect1.options.length > 0)
      fileSelect1.options[0].disabled = isFile1Selected;
    if (fileSelect2.options.length > 0)
      fileSelect2.options[0].disabled = isFile2Selected;
    ...
    if (
      selectedFile1 === state.lastFile1 &&
      selectedFile2 === state.lastFile2
    ) {
    ...
    console.log(
      `Selection changed. New selection: ${selectedFile1}, ${selectedFile2}`
    );
...
export function setDefaultSelections(elements, handleSelectionChangeCallback) {
  ...
  if (fileSelect1.options.length <= 1 || fileSelect2.options.length <= 1) {
    ...
    if (errorMessage) {
      errorMessage.textContent =
        "Cannot set default games: No game data found or UI not ready.";
  ...
  if (availableFiles.length >= 2) {
    ...
    console.log(
      `Default selections set (by filename): ${availableFiles[0]}, ${availableFiles[1]}`
    );
  } else if (availableFiles.length === 1) {
    fileSelect1.value = availableFiles[0];
    fileSelect2.value = availableFiles[0];
    console.log(
      `Single game available. Default selections set (by filename): ${availableFiles[0]}, ${availableFiles[0]}`
    );
  } else {
    if (errorMessage) {
    ...
    fileSelect2.disabled = true;
    console.error(
      "setDefaultSelections: No valid game files found after population."
    );
    return;
  }
