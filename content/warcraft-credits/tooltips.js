 ...
 */
export function setupD3Tooltips(
  nodeSelection,
  tooltipElement,
  personRolesMap,
  svgNode,
  ...
  const showTooltip = (d, screenPos) => {
    ...
    if (d.type === NODE_TYPE_PERSON) {
      ...
      if (roles && roles.size > 0) {
        htmlContent += `<br><span class="tooltip-info">Role(s): ${[...roles]
          .sort()
          .join(", ")}</span>`;
    ...
    if (tooltipRect.right > viewportWidth - 10) {
      tooltipElement.style.left = `${
        screenPos[0] - tooltipRect.width - tooltipPadding
      }px`;
    }
    if (tooltipRect.bottom > viewportHeight - 10) {
      tooltipElement.style.top = `${
        screenPos[1] - tooltipRect.height - tooltipPadding
      }px`;
    }
  ...
  nodeSelection.on("click", (event, d) => {
    ...
    if (isCtrlClick) {
      const currentTargetNode = d3.select(event.currentTarget);
      hideTooltipOnly();
      const wasAlreadyHighlighted = currentTargetNode.classed(
        "highlighted-department"
      );
      const wasHighlightActive = nodesGroup
        ? nodesGroup.classed("department-highlight-active")
        : false;
  ...
  d3.select(svgNode).on("click", (event) => {
    if (
      event.target === svgNode ||
      event.target === zoomLayer.node() ||
      event.target.classList.contains("zoom-layer")
    ) {
