...
function getActualDataPath(shortFilename) {
  if (
    !shortFilename ||
    typeof shortFilename !== "string" ||
    !shortFilename.includes(".")
  ) {
...
function getGameNameFromData(jsonData, filename) {
  if (!jsonData || typeof jsonData !== "object") {
    console.warn(
      `Cannot extract game name: Invalid JSON data provided for ${filename}.`
    );
    return null;
  }
  ...
  if (gameTitleMap[filename]) {
    console.warn(
      `Cannot extract game name: Expected one top-level key in ${filename}, found ${keys.length}. Trying config map.`
    );
    return null;
  }
  console.warn(
    `Cannot extract game name: Expected one top-level key in ${filename}, found ${keys.length}. Keys:`,
    keys
  );
  return null;
}
...
function calculateStats(jsonData, filename) {
  ...
  if (!gameName || !Array.isArray(jsonData[gameName])) {
    console.warn(
      `Worker: Could not find valid people array for stats calculation in ${filename} using key '${
        gameName || "unknown"
      }'. Checking top-level values.`
    );
    ...
    for (const key of topLevelKeys) {
      if (Array.isArray(jsonData[key])) {
        ...
        console.warn(
          `Worker: Found people array under unexpected key '${key}' in ${filename}. Using this for stats.`
        );
    ...
    if (!peopleArray) {
      console.error(
        `Worker: No valid people array found anywhere in ${filename} for stats calculation.`
      );
  ...
  if (personCount > 0 && uniqueRoles.size === 0) {
    console.warn(
      `Worker: ${filename} has ${personCount} people but no explicit non-empty roles listed. Reporting 0 unique roles.`
    );
  }
...
self.onmessage = async (event) => {
  ...
  console.log(
    `Worker (D3): Mapped to actual fetch paths: ${actualPath1}, ${actualPath2}`
  );
  ...
  try {
    ...
    try {
      if (isSameGame) {
        ...
        if (jsonData1) {
          ...
          if (!stats1)
            console.error(
              `Worker: Stat calculation failed for ${shortFilename1}`
            );
        } else {
          console.error(
            `Worker (D3): Failed to fetch single file: ${actualPath1}`
          );
        }
      } else {
        const results = await Promise.allSettled([
          fetchJsonFile(actualPath1),
          fetchJsonFile(actualPath2),
        ]);
        ...
        console.log(
          `Worker (D3): Fetched file 1 (${results[0].status}): ${actualPath1}`
        );
        console.log(
          `Worker (D3): Fetched file 2 (${results[1].status}): ${actualPath2}`
        );
        ...
        if (jsonData2) {
          ...
          if (!stats2)
            console.error(
              `Worker: Stat calculation failed for ${shortFilename2}`
            );
        }

        if (!jsonData1 && jsonData2)
          console.warn(
            `Worker (D3): Failed to fetch ${actualPath1}, proceeding with ${actualPath2}.`
          );
        if (jsonData1 && !jsonData2)
          console.warn(
            `Worker (D3): Failed to fetch ${actualPath2}, proceeding with ${actualPath1}.`
          );
      }
    } catch (fetchError) {
      console.error("Worker (D3) fetch execution error:", fetchError);
      throw new Error(
        `Failed during game data fetch attempt: ${fetchError.message}`
      );
    }

    if (!jsonData1 && !jsonData2) {
      throw new Error(
        `Failed to fetch data for both selections: ${shortFilename1}, ${shortFilename2}`
      );
    }
    ...
    const d3GraphData = processDataForD3(
      jsonData1,
      jsonData2,
      shortFilename1,
      shortFilename2,
    ...
    console.log(
      `Worker (D3): Finished data processing. Nodes: ${d3GraphData.nodes.length}, Links: ${d3GraphData.links.length}`
    );

    const personRolesMapData = Array.from(workerPersonRolesMap.entries()).map(
    ...
    if (
      allRoles.size === 0 &&
      d3GraphData.nodes.some((n) => n.type === "person")
    ) {
    ...
    if (
      !workerNormalizedRolePositions.has(DEFAULT_ROLE) &&
      allRoles.has(DEFAULT_ROLE)
    ) {
    ...
    const normalizedRolePositionsData = Array.from(
      workerNormalizedRolePositions.entries()
    );

    console.log(
      `Worker (D3): Posting back stats1: ${JSON.stringify(
        stats1
