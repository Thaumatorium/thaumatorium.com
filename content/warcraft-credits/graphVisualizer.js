...
function getNodeColor(d, allGameNodes) {
  if (d.type === NODE_TYPE_GAME) {
    ...
    const isDifferentGame2 =
      allGameNodes.length > 1 && d.gameIndex === 2 && d.id !== gameNode1?.id;
  ...
  if (d.type === NODE_TYPE_PERSON) {
    ...
    if (dynamicRoleColors.has(normalizedRole))
      return dynamicRoleColors.get(normalizedRole);
    if (
      normalizedRole &&
      normalizedRole !== DEFAULT_ROLE.toLowerCase().trim()
    ) {
...
function nodeRadius(d) {
  return d.type === NODE_TYPE_GAME
    ? EXAMPLE_GAME_NODE_RADIUS
    : EXAMPLE_PERSON_NODE_RADIUS;
}
...
export function visualizeGraphD3(
  ...
  const baseErrorMessage =
    "No common contributors or relevant data found for the selected combination.";
  if (!graphData || !graphData.nodes || graphData.nodes.length === 0) {
    if (
      !errorMessageElement.textContent ||
      !errorMessageElement.textContent.includes("hidden")
    ) {
      errorMessageElement.textContent =
        baseErrorMessage + " (No nodes to display).";
  ...
  const personNodesExist = graphData.nodes.some(
    (n) => n.type === NODE_TYPE_PERSON
  );
  if (!personNodesExist) {
    if (
      !errorMessageElement.textContent ||
      ...
      errorMessageElement.textContent =
        baseErrorMessage + " (No contributors to display).";
      errorMessageElement.style.display = "block";
      errorMessageElement.classList.add("error-message");
      errorMessageElement.classList.remove("warning-message");
    }
  ...
  const isSingleGameView =
    !gameNode2 || (gameNode1 && gameNode2 && gameNode1.id === gameNode2.id);
  ...
  if (isNaN(width) || isNaN(height) || width <= 0 || height <= 0) {
    ...
    console.error(
      "SVG container dimensions error:",
      svg.style("width"),
      svg.style("height")
    );
    return null;
  ...
  graphData.nodes.forEach((d) => {
    if (d.type === NODE_TYPE_GAME) {
      const xOffset = !isSingleGameView
        ? d.gameIndex === 1
          ? -width * 0.15
          : width * 0.15
        : 0;
      ...
      if (d.gameIndex === 2 && !isSingleGameView)
        game2InitialPos = { x: d.x, y: d.y };
      else if (d.gameIndex === 2 && isSingleGameView)
        game2InitialPos = game1InitialPos;
    }
    d.fx = null;
    d.fy = null;
  });
  graphData.nodes.forEach((d) => {
    if (d.type === NODE_TYPE_PERSON) {
      const role = d.primaryRole || DEFAULT_ROLE;
      const roleTargetPosNorm =
        normalizedRolePositions.get(role) || defaultRolePositionNorm;
      ...
      switch (d.category) {
        ...
        case CATEGORY_GAME1_ONLY:
          targetX =
            roleTargetX * (1 - interpolationWeight) +
            game1InitialPos.x * interpolationWeight;
          targetY =
            roleTargetY * (1 - interpolationWeight) +
            game1InitialPos.y * interpolationWeight;
          break;
        case CATEGORY_GAME2_ONLY:
          if (!isSingleGameView) {
            targetX =
              roleTargetX * (1 - interpolationWeight) +
              game2InitialPos.x * interpolationWeight;
            targetY =
              roleTargetY * (1 - interpolationWeight) +
              game2InitialPos.y * interpolationWeight;
          }
          break;
        case CATEGORY_BOTH:
          if (!isSingleGameView) {
            ...
            targetX =
              roleTargetX * (1 - bothInterpolationWeight) +
              midGameX * bothInterpolationWeight;
            targetY =
              roleTargetY * (1 - bothInterpolationWeight) +
              midGameY * bothInterpolationWeight;
          }
          break;
  ...
  const chargeForce = d3
    .forceManyBody()
    .strength(INITIAL_SIMULATION_CHARGE_STRENGTH);

  const centerForce = d3.forceCenter(width / 2, height / 2).strength(0.05);
  const forceX = d3
    .forceX()
    .strength((d) =>
      d.type === NODE_TYPE_PERSON
        ? INITIAL_ROLE_POSITIONING_FORCE_STRENGTH
        : 0.01
    )
    .x((d) => {
      if (d.type === NODE_TYPE_PERSON) {
    ...
    .on("zoom", (event) => {
      ...
      if (nodeSelection) {
        nodeSelection
          ...
          .classed("highlighted-department", false);
      }
